...
function setIssueImages(card, images) {
    ...
    images.map((item, index) => {
        issueImageSection.find(".imgSlides").append(` <div class="element ${index}">
            ...
            <div class="thumbnail" style="width:422px;background-color:#FEECEC">
            <p style="color: #594D77;
            float: left;
            width: 90%;
            border-right: 1px solid grey;">Issue Pending</p>
            <img src="images/edit.svg" style="width:25px;    position: relative;
            top: 10px;
            left: 14px;">
            </div>
         </div>`)
...
function getToken() {
        surl = window.location.href; // //http://www.share.buildworks.in/a8762fdb5d7bf561b832a358c22e9994//
